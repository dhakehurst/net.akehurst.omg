...
Normative language
- This document uses the keywords `SHALL` and `MAY` in accordance with RFC 2119. `SHALL` indicates a requirement and `MAY` indicates an option.
...
- [REQ-2.2.1] For each mapped model, a top-level API builder entry point SHALL be generated.
...
### 2.10 Identity and serialisation
- [REQ-2.10.1] `_identity` SHALL be the persistent identifier for an object and SHALL be persisted and reconstructed by Serialisation/Deserialisation.
  - [REQ-2.10.1.1] `_identity` SHALL be derived in this order of preference:
    - (a) composed from MOF property(ies) with `isID=true` when present,
    - (b) the source XMI `xmi:id` when the model originates from XMI and a stable `xmi:id` is available and the generator intends to preserve it,
    - (c) otherwise generated by the factory when an object is constructed.
  - [REQ-2.10.1.2] Generated `_identity` values SHALL be unique within a Factory instance at creation time.
  - [REQ-2.10.1.3] Serialisation SHALL persist `_identity` values such that deserialisation reconstructs objects with the same `_identity` and referential links can be restored.
  - [REQ-2.10.1.4] Factory implementations SHALL validate uniqueness of `_identity` on object creation or deserialisation; attempts to create or deserialise objects producing duplicate `_identity` values SHALL fail with an error diagnostic.
  - [REQ-2.10.1.5] `_identity` SHALL provide stable equality and hashing semantics: deserialised `_identity` values SHALL compare equal (via equals) to the persisted value and SHALL produce the same hashCode. Serialisation format MAY be non-textual provided equality/hashCode semantics are preserved on deserialisation.
...
- [REQ-3.2.5] `isDerivedUnion=true` realisation attributes SHALL not be mutable.
...
and realisation contracts. It specialises the general realisation rules with RAM-specific holder types,
callback behaviour, and generated mutator conventions.
